 ...
 * 
 * Some skeleton code has been provided for you. You must implement anythings
 * else you need for the pipeline simulation to work properly.
 * 
 * @param p the pipeline to simulate
 */
...
{
//     struct Pipeline* q = (struct Pipeline*)malloc(sizeof(struct Pipeline));

//      for (unsigned int i=0; i<PIPE_WIDTH;i++){     
//         {   q->pipe_latch[ID_LATCH][i]= p->pipe_latch[ID_LATCH][i];
...
// }
 ...
 for (unsigned int i=0; i<PIPE_WIDTH;i++){     
    ...
    uint8_t EX_rs1 = (p->pipe_latch[ID_LATCH][i].trace_rec.src1_needed == 1) && (p->pipe_latch[EX_LATCH][i].trace_rec.dest_needed == 1);
    uint8_t EX_rs2 =  (p->pipe_latch[ID_LATCH][i].trace_rec.src2_needed == 1) && (p->pipe_latch[EX_LATCH][i].trace_rec.dest_needed == 1);
    uint8_t MA_rs1 =  (p->pipe_latch[ID_LATCH][i].trace_rec.src1_needed == 1) && (p->pipe_latch[MA_LATCH][i].trace_rec.dest_needed == 1);
    uint8_t MA_rs2 = (p->pipe_latch[ID_LATCH][i].trace_rec.src2_needed == 1) && (p->pipe_latch[MA_LATCH][i].trace_rec.dest_needed == 1);
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        if(ID_valid&&EX_rs2&&(p->pipe_latch[EX_LATCH][i].valid == true) &&(p->pipe_latch[EX_LATCH][i].trace_rec.dest_reg == p->pipe_latch[ID_LATCH][i].trace_rec.src2_reg))
            {
                cout<<"case 1"<<endl;
                cout<<p->pipe_latch[ID_LATCH][i].op_id<<"    op__id1"<<endl;
                cout<<p->pipe_latch[EX_LATCH][i].op_id<<"    op__id2"<<endl;
                cout<<(p->pipe_latch[EX_LATCH][i].trace_rec.dest_reg == p->pipe_latch[ID_LATCH][i].trace_rec.src2_reg)<<endl;
            p->pipe_latch[EX_LATCH][i].stall = true;
            // p->pipe_latch[EX_LATCH][i].valid = false;
            cout << "one" << endl;}
        else if(ID_valid&& EX_rs1 && (p->pipe_latch[EX_LATCH][i].valid == true)&&(p->pipe_latch[EX_LATCH][i].trace_rec.dest_reg == p->pipe_latch[ID_LATCH][i].trace_rec.src1_reg) )
            {cout<<"case 2"<<endl;
            p->pipe_latch[EX_LATCH][i].stall = true;
            // p->pipe_latch[EX_LATCH][i].valid = false;
            cout << "one" << endl;}
        else if(ID_valid&& MA_rs1 &&   (p->pipe_latch[MA_LATCH][i].valid == true) &&  (p->pipe_latch[MA_LATCH][i].trace_rec.dest_reg == p->pipe_latch[ID_LATCH][i].trace_rec.src1_reg))
            {cout<<"case 3"<<endl;
            p->pipe_latch[EX_LATCH][i].stall = true;
            // p->pipe_latch[EX_LATCH][i].valid = false;
            cout << "one" << endl;}
        else if(ID_valid&& MA_rs2 && (p->pipe_latch[MA_LATCH][i].valid == true) &&  (p->pipe_latch[MA_LATCH][i].trace_rec.dest_reg == p->pipe_latch[ID_LATCH][i].trace_rec.src2_reg))
            {cout<<"case 4"<<endl;
            p->pipe_latch[EX_LATCH][i].stall = true;
            // p->pipe_latch[EX_LATCH][i].valid = false;
            cout << "one" << endl;}
        else if((p->pipe_latch[IF_LATCH][i].valid == true)&& (p->pipe_latch[ID_LATCH][i].valid == true) && (p->pipe_latch[EX_LATCH][i].trace_rec.cc_write == 1)&&(p->pipe_latch[ID_LATCH][i].trace_rec.cc_read==1) &&(p->pipe_latch[EX_LATCH][i].valid == true)&&(p->pipe_latch[ID_LATCH][i].trace_rec.op_type==3))
            {cout<<"case 5"<<endl;
            p->pipe_latch[EX_LATCH][i].stall = true;
            // p->pipe_latch[EX_LATCH][i].valid = false;
            cout << "one" << endl;}
        else if((p->pipe_latch[IF_LATCH][i].valid == true)&& (p->pipe_latch[ID_LATCH][i].valid == true) && (p->pipe_latch[MA_LATCH][i].trace_rec.cc_write == 1)&&(p->pipe_latch[ID_LATCH][i].trace_rec.cc_read==1) &&(p->pipe_latch[MA_LATCH][i].valid == true)&&(p->pipe_latch[ID_LATCH][i].trace_rec.op_type==3))
            {cout<<"case 6"<<endl;
            p->pipe_latch[EX_LATCH][i].stall = true;
            // p->pipe_latch[EX_LATCH][i].valid = false;
            cout << "one" << endl;}
...
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    // if((ID_valid&&EX_rs2&&(p->pipe_latch[EX_LATCH][i].valid == true) &&(p->pipe_latch[EX_LATCH][i].trace_rec.dest_reg == p->pipe_latch[ID_LATCH][i].trace_rec.src2_reg))||\
    // (ID_valid&& EX_rs1 && (p->pipe_latch[EX_LATCH][i].valid == true) &&(p->pipe_latch[EX_LATCH][i].trace_rec.dest_reg == p->pipe_latch[ID_LATCH][i].trace_rec.src1_reg))||\
    // (ID_valid&& MA_rs1 &&   (p->pipe_latch[MA_LATCH][i].valid == true) &&  (p->pipe_latch[MA_LATCH][i].trace_rec.dest_reg == p->pipe_latch[ID_LATCH][i].trace_rec.src1_reg))||\
    // (ID_valid&& MA_rs2 && (p->pipe_latch[MA_LATCH][i].valid == true) &&  (p->pipe_latch[MA_LATCH][i].trace_rec.dest_reg == p->pipe_latch[ID_LATCH][i].trace_rec.src2_reg))||\
    // (ID_valid && (p->pipe_latch[EX_LATCH][i].trace_rec.cc_write == 1)&&(p->pipe_latch[ID_LATCH][i].trace_rec.cc_read==1) &&(p->pipe_latch[EX_LATCH][i].valid == true)&&(p->pipe_latch[ID_LATCH][i].trace_rec.op_type==3))||\
    // (ID_valid && (p->pipe_latch[MA_LATCH][i].trace_rec.cc_write == 1)&&(p->pipe_latch[ID_LATCH][i].trace_rec.cc_read==1) &&(p->pipe_latch[MA_LATCH][i].valid == true)&&(p->pipe_latch[ID_LATCH][i].trace_rec.op_type==3)))
    // {
    //         p->pipe_latch[EX_LATCH][i].stall = true;
    // }
    // else
