...
void rob_print_state(ROB *rob)
{
    ...
    {
        printf("%5d ::  %7d", i, (int)rob->entries[i].inst.inst_num);
        printf(" %7d", rob->entries[i].valid);
        printf(" %7d", rob->entries[i].ready);
        printf(" %7d", rob->entries[i].exec);
        printf(" %10d", rob->entries[i].inst.src1_reg);
        printf(" %12d", rob->entries[i].inst.src1_ready);
        printf(" %14d", rob->entries[i].inst.src1_tag);
        printf(" %14d", rob->entries[i].inst.src2_reg);
        printf(" %15d", rob->entries[i].inst.src2_ready);
        printf(" %15d", rob->entries[i].inst.src2_tag);
        printf(" %16d", rob->entries[i].inst.dest_reg);
        printf(" %18d", rob->entries[i].inst.dr_tag);
...
bool rob_check_space(ROB *rob)
{
    ...
    //printf("head value %d\n", rob->head_ptr);
    //printf("tail value %d\n", rob->tail_ptr);
    //printf("head valid %d\n", rob->entries[rob->head_ptr].valid);
    //printf("head equal to tail %d\n", rob->tail_ptr == rob->head_ptr);
    int flag = (rob->tail_ptr == rob->head_ptr) && (rob->entries[rob->head_ptr].valid);
    //printf("flag value %d\n", flag);//start the flag should show 1 -enough, 0 not enough
    return flag;
...
int rob_insert(ROB *rob, InstInfo inst)
{
    ...
    bool enough = rob_check_space(rob);
    if (enough) {
        //printf("enough\n");
        return -1;
    ...
    else 
    {
        ...
        rob->entries[rob->tail_ptr].exec = false;
        rob->entries[rob->tail_ptr].ready = false;
        if (rob->tail_ptr + 1 > 31)
        {
            //printf("shit\n");
            rob->tail_ptr = 0;
            return 31;
        ...
        {
            rob->tail_ptr = rob->tail_ptr + 1;
            //printf("original value %d", rob->tail_ptr);
            return rob->tail_ptr - 1;
        }//return PRF ID
    }
